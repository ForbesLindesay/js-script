  ...
  return falafel(src, function (node) {
    if (node.type === 'CallExpression' || node.type === 'NewExpression') {
      ...
      if (node.callee.type === 'MemberExpression') {
        callee = '[' + node.callee.object.source() + ',' + (node.callee.computed ? node.callee.property.source() : JSON.stringify(node.callee.property.source())) + ']'
        innerCallee = 'callee[0][callee[1]]'
      } else {
        callee = node.callee.source()
        innerCallee = 'callee'
      }
      node.update('$await(function (callee' + innerArgs.map(function (a) { return ',' + a }).join('') + ') { return '
